	...
	function fnInsert(){
		$('#btnInsert').on('click', function(){
			location.href='/admin/saveMemberPage?value=' + ${value};
		})
	}
	// 삭제
	...
	function fnlistCountChange(){
		$('#pageUnit').change(function(){
			location.href="/admin/listMember?value=" + $("#pageUnit option:selected").val();
	...
	function fnSearch(){
		...
		$('#btnSearch').on('click', function(){
			...
			if( column.val() == 'MEMBER_NAME' || column.val() == 'MEMBER_PHONE' || column.val() == 'MEMBER_ROAD_ADDRESS') {
				location.href="/admin/searchMember?column=" + column.val() + "&query=" + query.val() + "&value=" + ${value};
			} else {
				location.href="/admin/searchMember?column=" + column.val() + "&begin=" + begin.val() + "&end=" + end.val() + "&value=" + ${value};
			}
	...
	function fnAreaChoice(){
		...
		$('#column').on('change', function(){
			if( $(this).val() == '' ) {
				...
				$('#btnSearchSection').css('padding-right', '-76px');
