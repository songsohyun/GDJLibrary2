	...
	function fnInsert(){
		$('#btnInsert').on('click', function(){
			location.href='/admin/saveDormantMemberPage?value=' + ${value};
		})
	}
	
	function fnlistCountChange(){
		$('#pageUnit').change(function(){
			location.href="/admin/listDormantMember?value=" + $("#pageUnit option:selected").val();
	...
	function fnAutoComplete(){
		...
		$('#query').on('keyup', function(){
			...
			$.ajax({  // DB에서 입력한 값으로 시작하는 값을 가져와서 보여 줌
				url: '/admin/autoCompleteDormantMember',
				type: 'get',
	...
	function fnSearch(){
		...
		$('#btnSearch').on('click', function(){
			...
			if( column.val() == 'MEMBER_NAME' || column.val() == 'MEMBER_PHONE' || column.val() == 'MEMBER_ROAD_ADDRESS') {
				location.href="/admin/searchDormantMember?column=" + column.val() + "&query=" + query.val() + "&value=" + ${value};
			} else {
				location.href="/admin/searchDormantMember?column=" + column.val() + "&begin=" + begin.val() + "&end=" + end.val() + "&value=" + ${value};
			}
	...
	function fnAreaChoice(){
		...
		$('#column').on('change', function(){
			if( $(this).val() == '' ) {
				...
				$('#btnSearchSection').css('padding-right', '-75px');
